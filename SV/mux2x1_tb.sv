...
module mux2x1_tb;
    ...
    // is DUT for design under test. Here we have 5 DUTs, but most testbenches
    // only have one.
    ...
    mux2x1_case DUT_CASE (
        ...
        // details of the port. Recommended to only use it in testbenches where the
    ...
    initial begin
        ...
        for (integer i = 0; i < 8; i = i + 1) begin
            ...
            //This version uses what is called a blocking assignment (=).
            //Therefore, values are updated immediately
            in0 = i[0];
            in1 = i[1];
            sel = i[2];
            
            //This commented out version instead uses a non-blocking assignment (<=), which means:
            //The assignment does not happen immediately
            //It is scheduled for the end of the current simulation time step
            //Therefore, we get a result that is technically correct but looks out of sync with the other values
            /*in0 <= i[0];
            in1 <= i[1];
            sel <= i[2];*/
