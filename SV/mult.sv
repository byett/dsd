...
module mult3 #(
    ...
    // unsigned. The prefix mult_func. basically states to find a generate
    ...
    assign product = mult_func.multiply(in0, in1);
    ...
    // is with a case (or if) generate block. If we give this block a label, we
    ...
    //
    // It is rare that you need to hierachically access anything within a generate block for
    // synthesizable code, though there are situations where hierarchical 
    // access provides a more elegant solution. Adding the labels does allow
    // for hierachical access by testbenches, which can be very useful, so the
    // recommendation is to add the labels even if you aren't using them in your
    // synthesizable code.
    generate
        case (IS_SIGNED)
            1'b0: begin : mult_func
                function automatic [INPUT_WIDTH*2-1:0] multiply(input [$bits(in0)-1:0] x, y);
                    return x * y;
                endfunction
            end
            1'b1: begin : mult_func
                function automatic [INPUT_WIDTH*2-1:0] multiply(input [$bits(in0)-1:0] x, y);
