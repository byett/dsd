...
// the rising clock else in the same block. This is a violation of synthesis
...
//
// IMPORTANT:
// In some of the examples, you will notice the use both blocking and non-blocking
...
module arch2 #(
    ...
    // NOTE: A common naming convention for registers is a _r suffix. We don't
...
module arch3 #(
    ...
    // Here we add a variable for in3_r since it no longer connects directly to
    // the output register.
    ...
    // Similarly, we assign out2 outside of the rising clock edge.
...
module arch3_2 #(
    ...
    always_ff @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            // reading from it in the following line.
            //
            // VHDL COMPARISON: VHDL standards do not allow reading from
...
module arch4_2 #(
    ...
    //
    // In general, this is the safest implementation strategy. You can 
    // occassionally mix blocking and non-blocking assignments
    // when it becomes convenient, but make sure to have a good
    // reason first. For example, when each block gets
    // so big that we can't see the combinational and sequential logic
    // on the screen at the same time. In that case, the intent of
    // the code can get lost, in which cause it is better to see all
...
module arch5 #(
    ...
    //
    // Due to being so error prone and potentially causing race conditions, it is
    // strongly recommended to avoid this style. See the next two architectures 
...
module arch5_3 #(
    ...
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            // assignment inside the block, the recommendation is to declare local
...
module arch6_bad #(
    ...
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            // create a register despite out1 not being used anywhere outside the 
...
//
// There are a wide variety of options for sequential logic coding styles.
...
//
// It is a good idea to always start with always_ff and non-blocking
...
// as long as it synthesizes to the intended circuit. You should also
// only use blocking assignments on locally declared variables to 
// avoid accidental creation of registers by reading the variable outside the
// block.
//
// A good threshold for switching to blocking assignments is when
// separating large amounts of code into multiple blocks would make it
// considerably harder to read and understand the code. If it is necessary to
// constantly jump back and forth between blocks to understand functionality, the
...
module seq_example #(
    ...
    // Uncomment the various module names to synthesize different architectures.
    arch1 #(.WIDTH(WIDTH)) arch (.*);
    //arch2 #(.WIDTH(WIDTH)) arch (.*);
    //arch3 #(.WIDTH(WIDTH)) arch (.*);
    //arch3_2 #(.WIDTH(WIDTH)) arch (.*);
    //arch4 #(.WIDTH(WIDTH)) arch (.*);
    //arch4_2 #(.WIDTH(WIDTH)) arch (.*);
    //arch4_warnings #(.WIDTH(WIDTH)) arch (.*);
    //arch5 #(.WIDTH(WIDTH)) arch (.*);
    //arch5_2 #(.WIDTH(WIDTH)) arch (.*);
    //arch5_3 #(.WIDTH(WIDTH)) arch (.*);
    //arch5_4 #(.WIDTH(WIDTH)) arch (.*);
    //arch6_bad #(.WIDTH(WIDTH)) arch (.*);
    //arch6 #(.WIDTH(WIDTH)) arch (.*);
