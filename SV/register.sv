...
module register_async_rst #(
    ...
    // Sequential logic should usually use the always_ff block. You can use a
    // normal always block, but always_ff enables tools to apply additional 
    // checks that ensure that every assignment synthesizes to flip-flops. Tools tend to
    // not really verify flip flops, but instead just
    ...
    // Other tutorial examples will show situations that are safe to mix 
...
module register_en_async_rst #(
    ...
    always_ff @(posedge clk or posedge rst) begin
        ...
        // Registers can have an enable or a load signal which must be
...
// Description: Implements a parameterized register with configuration 
...
module register_flexible #(
    ...
    // When targetting FPGAs, use asynchronous resets in most cases
...
//One example of a T flip-flop implemented with a asynchronous reset still present
module tff_async_rst #(
    parameter int WIDTH
) (
    input  logic             clk,
    input  logic             rst,
    input  logic [WIDTH-1:0] in, //This now controls how we toggle instead of directly controlling inputs
    output logic [WIDTH-1:0] out
);
    always_ff @(posedge clk or posedge rst) begin
        //If we were using VHDL, code like this wouldn't work
        //For that, anything declared as an output (in this case, the out signal)
        //cannot be used like an input. But it works and simplifies our code here
        out <= in ^ out;
        if (rst) out <= '0;
    end
endmodule  // tff_async_rst
...
module register #(
    ...
    parameter logic USE_ENABLE = 1'b0,
    parameter logic USE_ASYNC_RST = 1'b1
) (
    input  logic             clk,
    input  logic             rst,
    input  logic             en,
